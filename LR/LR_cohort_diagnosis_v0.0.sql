...
all_mh_diagnosis AS (
	SELECT 
		pmfu.date,
		pmfu.patient_id,
		pmfu.main_diagnosis AS diagnosis
	FROM psychiatrist_mhgap_follow_up pmfu 
	WHERE pmfu.main_diagnosis IS NOT NULL AND pmfu.main_diagnosis != 'None'
	UNION 
	SELECT 
		pmfu.date,
		pmfu.patient_id,
		pmfu.secondary_diagnosis AS diagnosis
	FROM psychiatrist_mhgap_follow_up pmfu 
	WHERE pmfu.secondary_diagnosis IS NOT NULL AND pmfu.secondary_diagnosis != 'None'
	UNION
	SELECT
		pmia.date,
		pmia.patient_id,
		pmia.main_diagnosis AS diagnosis
	FROM psychiatrist_mhgap_initial_assessment pmia 
	WHERE pmia.main_diagnosis IS NOT NULL AND pmia.main_diagnosis != 'None'
	UNION
	SELECT
		pmia.date,
		pmia.patient_id,
		pmia.secondary_diagnosis AS diagnosis 
	FROM psychiatrist_mhgap_initial_assessment pmia 
	WHERE pmia.secondary_diagnosis IS NOT NULL AND pmia.secondary_diagnosis != 'None'
	UNION 
	SELECT 
		n.date,
		d.patient_id, 
		d.diagnosis
	FROM diagnosis d 
	LEFT OUTER JOIN ncd n ON d.encounter_id = n.encounter_id)
SELECT 
	...
	amd.diagnosis
FROM all_mh_diagnosis amd
LEFT OUTER JOIN entry_exit_cte eec
	ON amd.patient_id = eec.patient_id
...
WHERE amd.date >= eec.entry_date AND (amd.date <= eec.discharge_date OR eec.discharge_date IS NULL)